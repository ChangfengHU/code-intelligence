...
class DeepSeekTTSService(private val config: TTSConfig) : TTSService {
    ...
    private val baseUrl = "https://api.deepseek.com/v1/audio/speech"
    
    override fun speak(text: String) {
        if (config.deepseekApiKey.isBlank()) {
            throw IOException("DeepSeek API Key未配置")
        }
        ...
        try {
            ...
            throw IOException("DeepSeek TTS失败: ${e.message}")
        }
    }
    ...
    private fun requestTTS(text: String): ByteArray {
        val json = JsonObject().apply {
            ...
            addProperty("voice", "alloy") // DeepSeek默认使用alloy声音
    ...
    private fun playAudio(audioData: ByteArray) {
        try {
            // 创建临时音频文件
            val tempFile = File.createTempFile("deepseek_tts_", ".mp3")
