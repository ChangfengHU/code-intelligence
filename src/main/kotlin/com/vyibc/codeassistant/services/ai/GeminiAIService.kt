...
class GeminiAIService(private val config: AIConfig) : AIService {
    ...
    override fun translateCode(code: String): String {
        ...
        try {
            ...
            throw IOException("Gemini代码翻译失败: ${e.message}")
        }
    }
    
    private fun requestTranslation(code: String): String {
        val userPrompt = "这是你需要翻译的代码：\n\n```\n$code\n```"
    ...
    private fun parseGeminiResponse(jsonResponse: String): String {
        try {
            ...
            // 智能清理：移除代码块标记但保持内部格式
            return cleanCodeBlockResponse(text)
            
        } catch (e: Exception) {
            throw IOException("解析Gemini响应失败: ${e.message}")
        }
    }
