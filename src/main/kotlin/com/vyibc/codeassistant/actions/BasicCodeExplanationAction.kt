...
class BasicCodeExplanationAction : AnAction("基础解释") {
    ...
    private fun handleSimpleTranslation(
        ...
        val stages = listOf("准备基础解释", "AI翻译处理中", "完成基础解释")
        ...
        progressService.runInBackground(
            ...
            onSuccess = { _ ->
                Messages.showInfoMessage("基础解释完成！", "基础代码解释")
            },
            onError = { ex ->
    ...
    private fun handleClassBasedTranslation(
        ...
        progressService.runInBackground(
            ...
            onError = { ex ->
                Messages.showErrorDialog("基础解释过程中出现错误: ${ex.message}", "错误")
            }
        )
    }
    ...
    override fun update(e: AnActionEvent) {
        ...
        if (hasSelection && editor != null) {
            ...
            e.presentation.text = when {
                lineCount <= 200 -> "基础解释 ($lineCount 行)"
                else -> {
                    val classCount = countClasses(selectedText ?: "")
                    if (classCount > 1) {
                        "基础解释 ($lineCount 行，$classCount 个类)"
                    } else {
                        "基础解释 ($lineCount 行)"
                    }
                }
