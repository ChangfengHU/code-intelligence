...
import com.vyibc.codeassistant.services.tts.SystemTTSService
import com.vyibc.codeassistant.config.TTSConfig
import java.io.File

class EnglishChinesePronunciationAction : AnAction("英中对照发音") {
    
    private val englishTtsService by lazy { TextToSpeechService() }
        ...
        progressService.runInBackground(
            ...
            task = { _, updateStage ->
                ...
                try {
                    ...
                    // 清空之前的队列
                    englishTtsService.stopSpeaking()
                    
                    for ((index, line) in lines.withIndex()) {
                        val cleanEnglishText = cleanTextForSpeech(line.trim())
                        if (cleanEnglishText.isNotEmpty()) {
                            
                            // 阶段2: 英语发音
                            updateStage(1, "正在发音英文: ${cleanEnglishText.take(20)}${if (cleanEnglishText.length > 20) "..." else ""}")
                            
                            // 发音英文
                            englishTtsService.speakEnglish(cleanEnglishText)
                            
                            // 等待英文发音完成
                            while (englishTtsService.isSpeaking() || englishTtsService.getQueueSize() > 0) {
                                Thread.sleep(200)
                            }
                            ...
                            if (chineseTranslation.isNotEmpty() && chineseTranslation != "翻译失败") {
                                // 阶段4: 中文发音
                                updateStage(3, "正在发音中文: ${chineseTranslation.take(20)}${if (chineseTranslation.length > 20) "..." else ""}")
                                
                                // 使用系统中文TTS发音
                                speakChinese(chineseTranslation)
                                
                                // 等待中文发音完成
                                Thread.sleep(Math.max(2000L, chineseTranslation.length * 300L)) // 根据文本长度动态调整等待时间
                            }
                    ...
                    javax.swing.SwingUtilities.invokeLater {
                        if (lines.size == 1) {
                            Messages.showInfoMessage("英中对照发音完成", "英中对照发音")
                        } else {
                            Messages.showInfoMessage("已完成 ${lines.size} 行英中对照发音", "英中对照发音")
                        }
                    }
                    
                } catch (ex: Exception) {
                    javax.swing.SwingUtilities.invokeLater {
                        Messages.showErrorDialog("英中对照发音失败: ${ex.message}", "错误")
                    }
                }
            }
        )
...
${'$'}synth.Dispose()
    ...
    override fun update(e: AnActionEvent) {
        ...
        // 显示状态
        if (englishTtsService.isSpeaking()) {
            e.presentation.text = "英中对照发音 (播放中...)"
        } else if (englishTtsService.getQueueSize() > 0) {
            e.presentation.text = "英中对照发音 (队列: ${englishTtsService.getQueueSize()})"
        } else {
            e.presentation.text = "英中对照发音"
        }
    }
